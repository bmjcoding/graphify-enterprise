...
def _html_styles() -> str:
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .node text { paint-order: stroke; stroke: #0f0f1a; stroke-width: 2px; }
...
const HYPEREDGES = __HYPEREDGES_JSON__;
...
function layoutNodes() {
  graphWidth = Math.max(container.clientWidth || 900, 640);
  graphHeight = Math.max(container.clientHeight || 700, 520);
  const visible = RAW_NODES.filter(n => !hiddenCommunities.has(n.community));
  const groups = new Map();
  visible.forEach(n => {
    const key = n.community === null || n.community === undefined ? -1 : n.community;
  ...
  const ring = Math.max(130, Math.min(graphWidth, graphHeight) * 0.32);
  entries.forEach(([cid, nodes], groupIndex) => {
    const groupAngle = entries.length === 1 ? 0 : (Math.PI * 2 * groupIndex / entries.length) - Math.PI / 2;
    const groupX = cx + Math.cos(groupAngle) * ring;
    const groupY = cy + Math.sin(groupAngle) * ring;
    const localRing = Math.max(36, Math.min(130, 16 + nodes.length * 5));
    nodes.forEach((n, i) => {
      const angle = nodes.length === 1 ? 0 : (Math.PI * 2 * i / nodes.length);
      n._x = groupX + Math.cos(angle) * localRing;
      n._y = groupY + Math.sin(angle) * localRing;
    });
  });
}
...
function renderGraph() {
  ...
  marker.appendChild(svgEl('path', { d: 'M0,0 L0,6 L7,3 z', fill: '#6b7280' }));
  defs.appendChild(marker);
  svg.appendChild(defs);

  HYPEREDGES.forEach(h => {
    const points = (h.nodes || []).map(id => nodeById.get(id)).filter(n => n && n._x !== undefined);
    if (points.length < 3) return;
    svg.appendChild(svgEl('polygon', {
      points: points.map(n => `${n._x},${n._y}`).join(' '),
      fill: '#6366f1',
      opacity: '0.08',
      stroke: '#6366f1',
      'stroke-width': '1',
  ...
  RAW_EDGES.forEach(edge => {
    ...
    if (!a || !b) return;
    if (hiddenCommunities.has(a.community) || hiddenCommunities.has(b.community)) return;
    const line = svgEl('line', {
      x1: a._x, y1: a._y, x2: b._x, y2: b._y,
      stroke: edge.confidence === 'EXTRACTED' ? '#94a3b8' : '#64748b',
      'stroke-width': edge.width || 1,
      'stroke-opacity': edge.confidence === 'EXTRACTED' ? '0.55' : '0.28',
      'stroke-dasharray': edge.confidence === 'EXTRACTED' ? '' : '5 5',
  ...
  RAW_NODES.forEach(n => {
    ...
    const group = svgEl('g', { class: selectedNodeId === n.id ? 'node selected' : 'node', tabindex: '0' });
    const radius = Math.max(5, Math.min(18, n.size || 8));
    group.appendChild(svgEl('circle', {
      cx: n._x, cy: n._y, r: radius,
      fill: n.color && n.color.background ? n.color.background : '#4E79A7',
      stroke: selectedNodeId === n.id ? '#ffffff' : '#111827',
      'stroke-width': selectedNodeId === n.id ? 3 : 1.5,
    }));
    ...
    group.addEventListener('keydown', event => {
      if (event.key === 'Enter' || event.key === ' ') focusNode(n.id);
    });
...
function showInfo(nodeId) {
  ...
  if (!n) return;
  const neighborIds = RAW_EDGES.flatMap(e => e.from === nodeId ? [e.to] : e.to === nodeId ? [e.from] : []);
...
renderGraph();
    ...
    return (
        ...
        .replace("__HYPEREDGES_JSON__", hyperedges_json)
    )
...
def to_html(
    ...
    # Build nodes list for the self-contained SVG renderer.
    vis_nodes = []
    for node_id, data in G.nodes(data=True):
...
<body>
<div id="graph"><svg id="graph-svg" role="img" aria-label="graphify knowledge graph"></svg></div>
<div id="sidebar">
  <div id="search-wrap">
...
def push_to_neo4j(
    ...
    """
    from graphify.security import validate_network_egress_endpoint
    validate_network_egress_endpoint(
        uri,
        purpose="Neo4j push",
