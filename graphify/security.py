...
_NETWORK_POLICIES = {"open", "allowlist", "deny"}


# ---------------------------------------------------------------------------
# Enterprise network egress policy
# ---------------------------------------------------------------------------
...
def enterprise_mode_enabled() -> bool:
    """Return True when fail-closed enterprise controls are enabled.

    Set GRAPHIFY_ENTERPRISE=1 in managed endpoint environments. In enterprise
    mode, outbound network access is denied unless GRAPHIFY_ALLOWED_HOSTS
    explicitly allows the destination host.
    """
    return _truthy_env("GRAPHIFY_ENTERPRISE")


def network_policy() -> str:
    """Return the active network policy: open, allowlist, or deny.

    GRAPHIFY_DISABLE_NETWORK=1 always wins. GRAPHIFY_EGRESS_POLICY can be set
    directly. If GRAPHIFY_ENTERPRISE=1 and no explicit policy is set, graphify
    uses allowlist mode.
    """
    if _truthy_env("GRAPHIFY_DISABLE_NETWORK"):
        return "deny"
    configured = os.environ.get("GRAPHIFY_EGRESS_POLICY", "").strip().lower()
    if configured:
        if configured not in _NETWORK_POLICIES:
            raise ValueError(
                "Invalid GRAPHIFY_EGRESS_POLICY. Expected one of: "
                + ", ".join(sorted(_NETWORK_POLICIES))
            )
        return configured
    return "allowlist" if enterprise_mode_enabled() else "open"


def _allowed_hosts() -> list[str]:
    raw = os.environ.get("GRAPHIFY_ALLOWED_HOSTS", "")
...
def validate_network_egress_endpoint(
    endpoint: str,
    *,
    purpose: str = "network egress",
    allowed_schemes: set[str] | None = None,
) -> str:
    """Validate an outbound destination against the enterprise egress policy.

    This is intentionally separate from validate_url(): it applies to any
    networked endpoint, including HTTPS model gateways, git remotes, and Neo4j
    bolt URIs. In default open-source mode, it is a no-op except for optional
    scheme validation. In enterprise mode, it fails closed unless the hostname
    appears in GRAPHIFY_ALLOWED_HOSTS.
    """
    ...
    if allowed_schemes is not None and scheme not in allowed_schemes:
        raise ValueError(
            f"Blocked {purpose}: scheme {scheme!r} is not allowed. "
    ...
    if policy == "deny":
        raise ValueError(
            f"Blocked {purpose}: outbound network access is disabled by "
            "GRAPHIFY_DISABLE_NETWORK or GRAPHIFY_EGRESS_POLICY=deny."
        )
    if not hostname:
        raise ValueError(
            f"Blocked {purpose}: endpoint has no hostname under "
    ...
    if not allowed:
        raise ValueError(
            f"Blocked {purpose}: GRAPHIFY_ALLOWED_HOSTS is empty while "
            "allowlist egress policy is active."
        )
    if not _host_allowed(hostname, allowed):
        raise ValueError(
            f"Blocked {purpose}: host {hostname!r} is not in "
...
def validate_url(url: str) -> str:
    ...
    hostname = parsed.hostname
    if not hostname:
        raise ValueError(f"Blocked URL without a hostname. Got: {url!r}")
    if hostname:
...
class _NoFileRedirectHandler(urllib.request.HTTPRedirectHandler):
    ...
    def redirect_request(self, req, fp, code, msg, headers, newurl):
        validate_network_egress_endpoint(newurl, purpose="redirect", allowed_schemes=_ALLOWED_SCHEMES)
...
def safe_fetch(url: str, max_bytes: int = _MAX_FETCH_BYTES, timeout: int = 30) -> bytes:
    ...
    """
    validate_network_egress_endpoint(url, purpose="URL fetch", allowed_schemes=_ALLOWED_SCHEMES)
    validate_url(url)
