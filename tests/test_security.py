...
def test_enterprise_mode_defaults_to_allowlist(monkeypatch):
    monkeypatch.setenv("GRAPHIFY_ENTERPRISE", "1")
    monkeypatch.delenv("GRAPHIFY_EGRESS_POLICY", raising=False)
    monkeypatch.delenv("GRAPHIFY_DISABLE_NETWORK", raising=False)
    assert enterprise_mode_enabled()
    assert network_policy() == "allowlist"


def test_egress_allowlist_blocks_unknown_host(monkeypatch):
    monkeypatch.setenv("GRAPHIFY_EGRESS_POLICY", "allowlist")
    monkeypatch.setenv("GRAPHIFY_ALLOWED_HOSTS", "internal-bedrock.example.com,github.com")
    with pytest.raises(ValueError, match="not in GRAPHIFY_ALLOWED_HOSTS"):
        validate_network_egress_endpoint("https://unknown.example.org/v1", purpose="test", allowed_schemes={"https"})


def test_egress_allowlist_allows_exact_and_wildcard(monkeypatch):
    monkeypatch.setenv("GRAPHIFY_EGRESS_POLICY", "allowlist")
    monkeypatch.setenv("GRAPHIFY_ALLOWED_HOSTS", "github.com,*.example.internal")
    assert validate_network_egress_endpoint("https://github.com/org/repo.git", allowed_schemes={"https"})
    assert validate_network_egress_endpoint("https://bedrock.example.internal/model", allowed_schemes={"https"})


def test_egress_deny_blocks_everything(monkeypatch):
    monkeypatch.setenv("GRAPHIFY_DISABLE_NETWORK", "1")
    with pytest.raises(ValueError, match="outbound network access is disabled"):
        validate_network_egress_endpoint("https://github.com/org/repo.git", allowed_schemes={"https"})
