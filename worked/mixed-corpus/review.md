...
**Pipeline:** detect → extract (AST) → build → cluster → analyze → query
...
All three queries ran against the real graph.json and returned relevant subgraphs.
...
## 5. Arabic Image OCR (via Claude vision)
...
1. **Arabic OCR requires zero special handling.** PIL creates the image, Claude reads it.
