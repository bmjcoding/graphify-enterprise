...
curl -fsSL <internal-mirror>/graphify/skill.md \
...
graphify sends semantic extraction content only through the approved assistant platform. Enterprise deployments must route this through the internal Amazon Bedrock Claude gateway or GitHub Copilot Enterprise. No telemetry, usage tracking, analytics, local run history, or query-answer memory is recorded.
